...
public class CreateAccountServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean isExist = ad.isExist(accounts, fullname);
        boolean isPass = password.equalsIgnoreCase(repassword);
        if (isExist) {
            request.setAttribute("exist", "Tài Khoản Đã Tồn Tại");
        } else if (!isPass) {
            request.setAttribute("repass", "Mật Khẩu Không Trùng Khớp Hãy Nhập Lại");
        } else {
            try {
                int phones = Integer.parseInt(phone);
                if (fullname != null && password != null && email != null && phone != null) {
                    ad.insertAccount(fullname, password, fullname, phones, email, address);
